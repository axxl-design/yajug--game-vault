  ...
  return (
    <Modal open={open} onClose={onClose} title={card.name} size="md">
      <div className="flex flex-col gap-3">
        ...
        {isWildcard(card) && (
          <>
            <p className="font-sans text-13 text-text-muted">Elegí color:</p>
        ...
        {isRent(card) && (
          <>
            <p className="font-sans text-13 text-text-muted">Cobrar de qué color:</p>
            <div className="flex flex-wrap gap-2">
              {(card.colors && card.colors.length > 0
                ...
                player.sets.some((s) => s.color === col && s.properties.length > 0) ? (
                  <div key={col} className="flex flex-col gap-1 rounded-6 border border-border p-2">
                    <span className="font-mono text-11 text-text-muted">{col}</span>
        ...
        {isBuilding(card) && (
          <>
            <p className="font-sans text-13 text-text-muted">Mejorar set completo:</p>
            <div className="flex flex-wrap gap-2">
              {player.sets
        ...
        {isAction(card) && card.actionName === 'factura' && (
          <>
            <p className="font-sans text-13 text-text-muted">A quién facturás 5M:</p>
            <div className="flex flex-wrap gap-2">
              {others.map((o) => (
                <Button
                  key={o.id}
        ...
        {isAction(card) && card.actionName === 'confiscacion' && (
          <>
            <p className="font-sans text-13 text-text-muted">Confiscar set completo:</p>
        ...
        {isAction(card) && card.actionName === 'trato_sucio' && (
          <>
            <p className="font-sans text-13 text-text-muted">Robar propiedad suelta:</p>
            <div className="flex flex-col gap-2">
              {others.flatMap((o) =>
                o.sets
        ...
        {isAction(card) && card.actionName === 'bloqueo' && (
          <p className="font-sans text-13 text-text-muted">
            Bloqueo se usa automáticamente cuando te atacan.
          </p>
        )}
